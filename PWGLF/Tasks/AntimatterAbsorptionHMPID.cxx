  ...
  void processMC(EventCandidates::iterator const& event, TrackCandidates const& tracks, aod::McParticles& mcParticles, aod::McCollisions const& mcCollisions)
  {
    ...
    for (auto track : tracks) {
      ...
      if (particle.pdgCode() == 211) {
        ...
        float xtogo = 0;
        if (!par_cov.getXatLabR(radius_hmpid, xtogo, prop->getNominalBz(), o2::track::DirType::DirOutward))
          continue;
        if (!prop->PropagateToXBxByBz(par_cov, xtogo, 0.95, 10, o2::base::Propagator::MatCorrType::USEMatCorrLUT))
          continue;
